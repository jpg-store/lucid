...
export class Tx {
  ...
  private async splitChange() {
    ...
    const mintValue =
      positiveMintAssets && C.Value.new_from_assets(positiveMintAssets);
    const negativeMintAssets = this.txBuilder.mint()?.as_negative_multiasset();
    const burnValue =
      negativeMintAssets && C.Value.new_from_assets(negativeMintAssets);

    let change = this.txBuilder
      .get_explicit_input()
      .checked_sub(this.txBuilder.get_explicit_output());

    if (mintValue) change = change.checked_add(mintValue);
    if (burnValue) change = change.checked_sub(burnValue);
